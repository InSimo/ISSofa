...
protected :
    ...
    void LDL_ordering_metis(int n,int * M_colptr,int * M_rowind,int * perm,int * invperm) {
        ...
        //we clear tran_countvec because we use it now to know how many values are written on each line
        tran_countvec.clear();
        tran_countvec.resize(n);
    ...
    void LDL_ordering(int n,int * M_colptr,int * M_rowind,int * perm,int * invperm,int orderingMode) {
        switch (orderingMode) {
        case 0 :
    ...
    void factorize(int n,int * M_colptr, int * M_rowind, Real * M_values, SparseLDLImplInvertData<VecInt,VecReal> * data) {
        ...
        bool new_factorization_needed = data->P_colptr.size() == 0 || data->P_rowind.size() == 0 || data->orderingMode != orderingMode
			|| CSPARSE_need_symbolic_factorization(n, M_colptr, M_rowind, data->n, (int *) data->P_colptr.data(),(int *) data->P_rowind.data());
        ...
        if (new_factorization_needed) {
            ...
            //ordering function
            LDL_ordering(data->n,M_colptr,M_rowind,data->perm.data(),data->invperm.data(), orderingMode);

            data->Parent.clear();
            data->Parent.resize(data->n);

            //symbolic factorization
            LDL_symbolic(data->n,M_colptr,M_rowind,data->L_colptr.data(),data->perm.data(),data->invperm.data(),data->Parent.data());
        ...
        LDL_numeric(data->n,M_colptr,M_rowind,M_values,colptr,rowind,values,D,data->perm.data(),data->invperm.data(),data->Parent.data());
