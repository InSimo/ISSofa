...
namespace collision
{
    ...
    public:
        ...
        typedef std::unordered_map<std::pair<int,int>,CollidingPair, sofa::helper::HashPair> umap_collision;
                ...
                for(int j = 0 ; j < sofa::core::CollisionModel::ENUM_TYPE_SIZE ; ++j){
                    if(_order[i][j] > 0){
                        ...
                        for(umap_collision::iterator it = _coll_pairs[i][j].begin() ; it != _coll_pairs[i][j].end() ; ++it){
                            auto& output = phase->getDetectionOutputs(it->second.elem1.getCollisionModel(),it->second.elem2.getCollisionModel());
                            ei->beginIntersect(it->second.elem1.getCollisionModel(),it->second.elem2.getCollisionModel(),output);
                            ei->intersect(it->second.elem1,it->second.elem2,output.first);
                        }
                    }
                }
            }
