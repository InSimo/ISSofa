...
{
    result = 0;
    MechanicalVDotVisitor(params /* PARAMS FIRST */, a,b,&result).execute( ctx, executeVisitor.precomputedTraversalOrder );
}

void VectorOperations::v_norm( sofa::core::ConstMultiVecId a, unsigned l)
{
    MechanicalVNormVisitor vis(params, a,l);
    vis.execute( ctx, executeVisitor.precomputedTraversalOrder );
