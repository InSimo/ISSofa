...
typedef ::testing::Types<
            sofa::defaulttype::MapMapSparseMatrix<sofa::defaulttype::Vec1Types::Deriv>,
            sofa::defaulttype::MapMapSparseMatrix<sofa::defaulttype::Vec2Types::Deriv>,
            sofa::defaulttype::MapMapSparseMatrix<sofa::defaulttype::Vec3Types::Deriv>,
            sofa::defaulttype::MapMapSparseMatrix<sofa::defaulttype::Rigid2Types::Deriv>,
...
//////////////////////////////////////////////////
template <typename TMatrix>
sofa::helper::vector<typename TMatrix::KeyType> Populate(TMatrix& matrix, unsigned int nbLine, unsigned int nbCol)
{
    typedef TMatrix Matrix;
    typedef typename TMatrix::KeyType KeyType;

    sofa::helper::vector<KeyType> lineIndices;
...
TYPED_TEST(SparseMatrixTest, CheckThatALineCanBeCorrectlyRetrievedByItsIndex)
{
    ...
    sofa::helper::vector<typename Matrix::KeyType> lindices = TestHelpers::Populate(matrix, 3u, 1u);
...
TYPED_TEST(SparseMatrixTest, CheckThatTrueIsReturnedWhenComparingTwoRowConstIteratorsPointingToTheSameLine)
{
    typedef TypeParam Matrix;
    Matrix matrix;
    sofa::helper::vector<typename Matrix::KeyType> lindices = TestHelpers::Populate(matrix, 1u, 1u);
...
TYPED_TEST(SparseMatrixTest, CheckThatTrueIsReturnedWhenComparingTwoRowConstIteratorsPointingToDistinctLinesForInequality)
{
    typedef TypeParam Matrix;
    Matrix matrix;
    sofa::helper::vector<typename Matrix::KeyType> lindices = TestHelpers::Populate(matrix, 2u, 1u);
...
TYPED_TEST(SparseMatrixTest, CheckThatALineCanBeIdenticallyRepopulatedAfterItHasBeenClearedInAOneLineMatrix)
{
    ...
    // Clear
    {
        matrix.clearRowBloc(line.rowIndex);
    }
...
TYPED_TEST(SparseMatrixTest, CheckThatALineCanBeRepopulatedWithDifferentColumnsIndicesAndDataAfterItHasBeenClearedInAOneLineMatrix)
{
    ...
    // Clear
    {
        matrix.clearRowBloc(line1.rowIndex);
    }
...
TYPED_TEST(SparseMatrixTest, CheckThatALineCanBeIdenticallyRepopulatedAfterItHasBeenClearedInAMultiLinesMatrix)
{
    ...
    // Clear
    {
        matrix.clearRowBloc(line2.rowIndex);
    }
...
TYPED_TEST(SparseMatrixTest, CheckThatTheSizeOfALineIsDecrementedByOneWhenAnElementIsErased)
{
    ...
    matrix.clearColBloc(line.data2.index);
...
TYPED_TEST(SparseMatrixTest, CheckThatThePastTheEndIteratorIsReturnedWhenTryingToAccessAnErasedElementInALine)
{
    ...
    {
        matrix.clearColBloc(line.data3.index);
    }
...
TYPED_TEST(SparseMatrixTest, CheckThatTryingToEraseAColumnThatDoesNotExistInALineLeavesTheSizeOfTheRowUnchanged)
{
    ...
    matrix.clearColBloc(line.data3.index + 1);
    EXPECT_EQ(expectedSize, itRow.row().size());
}
...
TYPED_TEST(SparseMatrixTest, CheckThatNewElementsCanBeInsertedIntoARowAfterAllOthersHaveBeenErased)
{
...

    {
        ...
        {
            SOFA_ASSERT(line.initialDataCount == 3);
            matrix.clearColBloc(line.data1.index);
            matrix.clearColBloc(line.data2.index);
            matrix.clearColBloc(line.data3.index);
        }
    }
...
TYPED_TEST(CompressedRowSparseMatrixConstraintTest, checkRowDeletionUntilMatrixIsEmpty)
{
    ...
    typedef typename ConstraintMatrix::Bloc Bloc;
    ...
    Bloc bTwo;
    Bloc bThree;
    for (std::size_t i=0; i<Bloc::size(); ++i)
    {
        bOne[i] = 1;
        bTwo[i] = 2;
        bThree[i] = 3;
    ...
    matrix.setBloc(0, 0, bOne);
    matrix.setBloc(0, 2, bOne);
    matrix.setBloc(0, 3, bOne);

    matrix.setBloc(1, 1, bTwo);

    matrix.setBloc(2, 1, bThree);
    matrix.setBloc(2, 3, bThree);
    ...
    ASSERT_EQ(matrix.getBloc(0, 0), bOne);
    ASSERT_EQ(matrix.getBloc(0, 2), bOne);
    ASSERT_EQ(matrix.getBloc(0, 3), bOne);

    ASSERT_EQ(matrix.getBloc(1, 1), bEmpty);

    ASSERT_EQ(matrix.getBloc(2, 1), bThree);
    ASSERT_EQ(matrix.getBloc(2, 3), bThree);
    ...
    ASSERT_EQ(matrix.getBloc(0, 0), bEmpty);
    ASSERT_EQ(matrix.getBloc(0, 2), bEmpty);
    ASSERT_EQ(matrix.getBloc(0, 3), bEmpty);
    ...
    ASSERT_EQ(matrix.getBloc(2, 1), bEmpty);
    ASSERT_EQ(matrix.getBloc(2, 3), bEmpty);
