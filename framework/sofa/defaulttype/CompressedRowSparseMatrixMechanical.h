...
public:
    ...
    void fullDiagonal()
    {
        SOFA_IF_CONSTEXPR (Policy::LogTrace) this->logCall(FnEnum::fullDiagonal);
        compress();
        Index ndiag = 0;
    ...
    {
        SOFA_IF_CONSTEXPR (Policy::Verbose)
        {
    ...
    **/
    SReal element(Index i, Index j) const override
    {
        ...
        SOFA_IF_CONSTEXPR (!Policy::StoreLowerTriangularBloc) if ((i / NL) > (j / NC)) std::swap(i,j);
    ...
    **/
    ...
    {
        SOFA_IF_CONSTEXPR (Policy::LogTrace) this->logCall(FnEnum::setVal, i, j, v);
        SOFA_IF_CONSTEXPR (Policy::Verbose) std::cout << this->Name()  << "("<<rowSize()<<","<<colSize()<<"): element("<<i<<","<<j<<") = "<<v<<std::endl;
        SOFA_IF_CONSTEXPR (Policy::Check)
        {
            if (i >= rowSize() || j >= colSize())
            {
        ...
        SOFA_IF_CONSTEXPR (!Policy::StoreLowerTriangularBloc) if ((i / NL) > (j / NC)) return;

        Index bi=0, bj=0; split_row_index(i, bi); split_col_index(j, bj);
        traits::vset(*this->wbloc(i, j, true), bi, bj, static_cast<Real>(v) );
        SOFA_IF_CONSTEXPR (Policy::Verbose) std::cout << this->Name()  << "("<<this->rowBSize()<<"*"<<NL<<","<<this->colBSize()<<"*"<<NC<<"): bloc("<<i<<","<<j<<")["<<bi<<","<<bj<<"] = "<<v<<std::endl;
    }
    ...
    **/
    ...
    {
        SOFA_IF_CONSTEXPR (Policy::LogTrace) this->logCall(FnEnum::addVal, i, j, v);
        SOFA_IF_CONSTEXPR (Policy::Verbose) std::cout << this->Name()  << "("<<rowSize()<<","<<colSize()<<"): element("<<i<<","<<j<<") += "<<v<<std::endl;
        SOFA_IF_CONSTEXPR (Policy::Check)
        {
            if (i >= rowSize() || j >= colSize())
    ...
    **/
    ...
    {
        SOFA_IF_CONSTEXPR (Policy::LogTrace) this->logCall(FnEnum::setValId, i, j, rowId, colId, v);
        SOFA_IF_CONSTEXPR (Policy::Verbose) std::cout << this->Name()  << "("<<rowSize()<<","<<colSize()<<"): element("<<i<<","<<j<<") = "<<v<<std::endl;
        SOFA_IF_CONSTEXPR (Policy::Check)
        {
            if (i >= rowSize() || j >= colSize())
    ...
    **/
    ...
    {
        SOFA_IF_CONSTEXPR (Policy::LogTrace) this->logCall(FnEnum::addValId, i, j, rowId, colId, v);
        SOFA_IF_CONSTEXPR (Policy::Verbose) std::cout << this->Name()  << "("<<rowSize()<<","<<colSize()<<"): element("<<i<<","<<j<<") += "<<v<<std::endl;
        SOFA_IF_CONSTEXPR (Policy::Check)
        {
            if (i >= rowSize() || j >= colSize())
    ...
    **/
    ...
    {
        SOFA_IF_CONSTEXPR (Policy::LogTrace) this->logCall(FnEnum::clearIndex, i, j);
        SOFA_IF_CONSTEXPR (Policy::Verbose) std::cout << this->Name()  << "("<<rowSize()<<","<<colSize()<<"): element("<<i<<","<<j<<") = 0"<<std::endl;
        SOFA_IF_CONSTEXPR (Policy::Check)
        {
            if (i >= rowSize() || j >= colSize())
            {
    ...
    **/
    ...
    {
        SOFA_IF_CONSTEXPR (Policy::LogTrace) this->logCall(FnEnum::clearRow, i);
        SOFA_IF_CONSTEXPR (Policy::Verbose) std::cout << this->Name()  << "("<<rowSize()<<","<<colSize()<<"): row("<<i<<") = 0"<<std::endl;
        SOFA_IF_CONSTEXPR (Policy::Check)
        {
            if (i >= rowSize())
            {
        ...
        SOFA_IF_CONSTEXPR (Policy::AutoCompress) this->compress(); /// If AutoCompress policy is activated, we neeed to be sure not missing btemp registered value.
    ...
    **/
    ...
    {
        SOFA_IF_CONSTEXPR (Policy::LogTrace) this->logCall(FnEnum::clearCol, j);
        SOFA_IF_CONSTEXPR (Policy::Verbose) std::cout << this->Name()  << "("<<rowSize()<<","<<colSize()<<"): col("<<j<<") = 0"<<std::endl;
        SOFA_IF_CONSTEXPR (Policy::Check)
        {
            if (j >= colSize())
            {
...
public:
    ...
    {
        SOFA_IF_CONSTEXPR (Policy::AutoCompress) compress();
    ...
    virtual BlockAccessor blocCreate(Index i, Index j)
    {
        ...
        if (this->sortedFind(this->rowIndex, i, rowId))
        {
            ...
            if (this->sortedFind(this->colsIndex, rowRange, j, colId))
            {
                SOFA_IF_CONSTEXPR (Policy::Verbose) std::cout << this->Name()  << "("<<this->rowBSize()<<"*"<<NL<<","<<this->colBSize()<<"*"<<NC<<"): bloc("<<i<<","<<j<<") found at "<<colId<<" (line "<<rowId<<")."<<std::endl;
                return createBlockAccessor(i, j, colId);
            }
        }
        if (this->btemp.empty() || this->btemp.back().l != i || this->btemp.back().c != j)
        {
            SOFA_IF_CONSTEXPR (Policy::Verbose) std::cout << this->Name()  << "("<<rowSize()<<","<<colSize()<<"): new temp bloc ("<<i<<","<<j<<")"<<std::endl;
...
/// @{
    ...
    void taddMulTranspose(V1& res, const V2& vec) const
    {
        ...
        SOFA_IF_CONSTEXPR (Policy::IsAlwaysSymetric) /// In symetric case this^T = this
        {
            taddMul(res, vec);
            return;
        }
