...
*/
  ...
  real& x() { static_assert(N >= 1, "N must be >= 1"); return this->elems[0]; }
  /// Special access to second element.
  real& y() { static_assert(N >= 2, "N must be >= 2"); return this->elems[1]; }
  /// Special access to third element.
  real& z() { static_assert(N >= 3, "N must be >= 3"); return this->elems[2]; }
  /// Special access to fourth element.
  real& w() { static_assert(N >= 4, "N must be >= 4"); return this->elems[3]; }

  /// Special const access to first element.
  const real& x() const { static_assert(N >= 1, "N must be >= 1"); return this->elems[0]; }
  /// Special const access to second element.
  const real& y() const { static_assert(N >= 2, "N must be >= 2"); return this->elems[1]; }
  /// Special const access to third element.
  const real& z() const { static_assert(N >= 3, "N must be >= 3"); return this->elems[2]; }
  /// Special const access to fourth element.
  const real& w() const { static_assert(N >= 4, "N must be >= 4"); return this->elems[3]; }
