...
class Mat : public fixed_array<Vec<C,real>,L>
{
 ...
 template<int L2, int C2,typename real2> void operator=(const Mat<L2,C2,real2>& m)
  ...
  Line& x() { static_assert(L >= 1, "L must be >= 1"); return this->elems[0]; }
  /// Special access to second line.
  Line& y() { static_assert(L >= 2, "L must be >= 2"); return this->elems[1]; }
  /// Special access to third line.
  Line& z() { static_assert(L >= 3, "L must be >= 3"); return this->elems[2]; }
  /// Special access to fourth line.
  Line& w() { static_assert(L >= 4, "L must be >= 4"); return this->elems[3]; }

  /// Special access to first line (read-only).
  const Line& x() const { static_assert(L >= 1, "L must be >= 1"); return this->elems[0]; }
  /// Special access to second line (read-only).
  const Line& y() const { static_assert(L >= 2, "L must be >= 2"); return this->elems[1]; }
  /// Special access to thrid line (read-only).
  const Line& z() const { static_assert(L >= 3, "L must be >= 3"); return this->elems[2]; }
  /// Special access to fourth line (read-only).
  const Line& w() const { static_assert(L >= 4, "L must be >= 4"); return this->elems[3]; }

  /// Set matrix to identity.
  void identity()
  {
    static_assert(L == C, "L and C must be equal");
    clear();
    for (int i=0;i<L;i++)
      this->elems[i][i]=1;
  }
